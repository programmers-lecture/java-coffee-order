...
- FoodCategory: 음식 카테고리에 대한 정보를 저장하는 Enum 객체
- Kiosk: 고객이 입력한 주문 혹은 쿠폰적립여부을 입력받고 출력하며, 주문정보와 쿠폰정보를 카페에게 전달하고 전달받는 객체
- Customer: 현재 주문하는 주체이며 휴대폰정보를 저장하여 이전에 왔던 고객인지 알 수 있는 객체
- Cafe: 키오스크에서 받은 모든 주문정보와 고객의 쿠폰정보를 전달받아 저장하고 키오스크에 전달하는 객체
- Coupon: 적립된 쿠폰 수량을 저장하고 있는 객체
- Food: 음식 카테고리, 번호, 이름, 가격 정보를 가지고 있는 객체
- Menu: 카페에서 제공하는 음식들의 리스트 정보를 가지고 있는 객체
- Order: 주문한 음식, 주문한 고객, 주문한 음식 갯수 정보를 가지고 있는 객체